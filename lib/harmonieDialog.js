...
export function createHarmonieDialogController({ harmonieCalculator, getState, getCheckedMagicTalentIds }) {
	...
	function renderHarmonieDialog() {
		...
		const totalContainer = totalEl?.closest('.harmonie-dialog-total');
		...
		// En dessous du package de départ, la formule affiche un ajustement négatif peu lisible : on la masque.
		const isTotalMeaningful = total >= startingPackage;
		if (totalContainer) {
			totalContainer.hidden = !isTotalMeaningful;
		}
		if (isTotalMeaningful) {
