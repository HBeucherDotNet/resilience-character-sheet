export function createSortDialogController({ getState, symbols, sorts, normalizePlaceholderToken, resolveScoreStep }) {
	...
	function bindSortDialog() {
		...
		const scoreInputs = form ? Array.from(form.querySelectorAll('.sort-dialog-input')) : [];
		...
		const sortDialogButtons = form ? Array.from(form.querySelectorAll('.sort-dialog-btn')) : [];
