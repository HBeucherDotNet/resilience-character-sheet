...
export function createOptionDetailContent({
	...
	detail.className = ['mobile-step-detail-content', saison].filter(Boolean).join(' ');
...
export function createOptionCard({
	...
	if (donPlacement === 'after-desc') {
		detailContent?.querySelectorAll('.don').forEach(node => option.appendChild(node.cloneNode(true)));
	}
